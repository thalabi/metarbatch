...
public interface MetarRepository extends JpaRepository<Metar, MetarPk> {
	...
	@Query(value = """
			select
			 ...
			 from metar
	     where station_id in (:stationIdCollection)
	       and observation_time between :fromObservationTime and :toObservationTime
			order by station_id, observation_time
			""", nativeQuery = true)
		List<Metar> findBetweenObservationTimes(Collection<String> stationIdCollection, LocalDateTime fromObservationTime, LocalDateTime toObservationTime);
